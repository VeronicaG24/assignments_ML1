# ----------------------------------------------------------------------------------------------------------------------
# Naive Bayes Classifier - Logarithmic transformation
# ----------------------------------------------------------------------------------------------------------------------
# Veronica Gavagna
# ----------------------------------------------------------------------------------------------------------------------
#
...
##############################################################
def log_check_testSet_values(no_copy, test_set):
...
##############################################################
def log_get_unique_values(test_s, trainingSet, targetIndex):
    target_list = list()
    no_copy = list()
    dim_max_values = 0
    for j in range(trainingSet.shape[1]):
        elements = list()
    ...
    test_set_ok = log_check_testSet_values(no_copy, test_s)
...
# training set with the logarithmic transformation
# RETURN:
#   3D matrix with all the likelihood probabilities
##############################################################
def log_calculate_lh_probabilities(training_set, targetIndex, testSet):
    rows_training_set, columns_training_set = training_set.shape

    header, target, dim_max_values, test_set_ok = log_get_unique_values(testSet, training_set, targetIndex)
    ...
    for k in range(0, target.shape[1], 1):
        ...
        prob_target[k] = np.log((total_target[k] / training_set.shape[0]))
    ...
    for k in range(0, target.shape[1], 1):
        for j in range(0, columns_training_set, 1):
            for h in range(0, dim_max_values, 1):
                for i in range(0, rows_training_set, 1):
                    if header[j, h] == 0:
                ...
                likelihood_prob[k, j, h] = np.log((total_value[k, h] / total_target[k]))
            total_value = np.zeros((target.shape[1], dim_max_values), dtype=int)
...
##############################################################
def log_test_NB_classifier(testSet, targetValues, possibleValues, likelihoodProb, targetIndex):
    posterior_prob = np.zeros((testSet.shape[0], targetValues.shape[1]), dtype=float)
...
#   the error rate
##############################################################
def log_compute_error_rate(testSet_old, targetIndex, testSet):
    error_rate = 0.
